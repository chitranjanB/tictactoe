...
public class Player {
	...
	public void move(String p)
	{
		...
		while(true)
		{
			...
			if(row>0 && row<=gameBoard.getSizeOfSquare() && col>0 && col <=gameBoard.getSizeOfSquare()) //if row or col is not > 0, input 0 works.
			{
				
			
				if(!gameBoard.fillSymbol(row, col, p))
				{
				System.out.println("This cell is already filled. Please re enter the cell: "); // write if condition to check alreday filled cell
			}
				else
				{
